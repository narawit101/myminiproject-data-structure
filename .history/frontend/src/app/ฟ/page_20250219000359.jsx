...
export default function BudgetAllocation() {
  ...
  useEffect(() => {
    fetch("/allocation/get") // เรียก API ดึงข้อมูลทุกโครงการ
      .then((res) => res.json())
      .then((data) => {
                ...
                <tr key={index}>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => handleEdit(budgetId, index, "allocation_percentage", e.target.value)}
                    />
                  </td>
                  <td>{allocation.allocated_amount.toFixed(2)}</td>
                  <td>
                    <input
                      type="number"
                      value={editedAllocations[`${budgetId}-${index}`]?.max_recipients ?? allocation.max_recipients}
                      onChange={(e) => handleEdit(budgetId, index, "max_recipients", e.target.value)}
                    />
                  </td>
                  <td>{allocation.amount_per_person ? allocation.amount_per_person.toFixed(2) : "-"}</td>
                  <td>
