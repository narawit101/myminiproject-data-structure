...
export default function CheckTargetPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
      <h1 className="text-2xl font-bold mb-4">ตรวจสอบกลุ่มเป้าหมายและวันรับเงิน</h1>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        {result && (
          <div className="mt-4 p-4 bg-green-100 border border-green-400 rounded">
            <p><strong>ชื่อ:</strong> {result.fname} {result.lname}</p>
            <p><strong>อายุ:</strong> {result.age} ปี</p>
            <p><strong>กลุ่มเป้าหมาย:</strong> {result.target_group || "ไม่เข้าข่าย"}</p>
            <p><strong>วันรับเงิน:</strong> {result.distribution_date || "ยังไม่มีข้อมูล"}</p>
          </div>
        )}
      </div>
    </div>
