...
export default function Dashboard() {
    ...
    const handleEdit = async (id) => {
        if (!newProjectName.trim() || !newTotalBudget) return; // ✅ เช็กว่า total_budget มีค่าหรือไม่
        ...
        try {
            const response = await fetch(`http://localhost:5000/budget/update/${id}`, {
                ...
                body: JSON.stringify({ 
                    ...
                    total_budget: newTotalBudget  // ✅ เพิ่ม total_budget
                }),
            });
    
            const result = await response.json();
            console.log("Update Response:", result); // ✅ Debug ค่าที่ส่งกลับมา
            ...
            setEditProject(null);
            setNewProjectName("");
            setNewTotalBudget(""); // ✅ ล้างค่า input budget หลังอัปเดต
                        ...
                        <tr key={budget.id}>
                            ...
                            <td>
                                {editProject === budget.id ? (
                                    ...
                                    <button onClick={() => setEditProject(budget.id)}>✏ Edit</button>
                                )}
