      ...
      <div className="bg-gray-100 flex flex-col items-center p-6">
        ...
        <div className="overflow-x-auto w-full max-w-4xl">
          <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <tbody>
  {budgets.map((budget) => {
    const remainingBudget = budget.remaining_budget; // Calculate remaining budget
    return (
      <tr key={budget.id} className="border-b">
        ...
        <td className="py-2 px-4">
          {editProject === budget.id ? (
            ...
            `${budget.total_budget.toLocaleString()} ฿` // แสดง total_budget พร้อมสัญลักษณ์บาท
          )}
        </td>
        <td className="py-2 px-4">
          {`${remainingBudget.toLocaleString()} ฿`} {/* แสดง Remaining Budget พร้อมสัญลักษณ์บาท */}
        </td>
