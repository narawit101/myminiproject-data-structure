...
export default function Dashboard() {
    ...
    const fetchBudgets = async () => {
        try {
            const response = await fetch("http://localhost:5000/check-target/:national_id", {
    ...
    const handleEdit = async (id) => {
        // Ensure that newProjectName and newYear are non-empty strings, and newTotalBudget is a valid number
        if (!newProjectName || !newYear || isNaN(newTotalBudget) || newTotalBudget === "") return;
        ...
        try {
            const response = await fetch(`http://localhost:5000/budget/update/${id}`, {
                ...
                body: JSON.stringify({ 
                    year: parseInt(newYear), // Ensure year is an integer
                    project_name: newProjectName, 
                    total_budget: parseFloat(newTotalBudget) // Ensure total_budget is a float
                }),
            });
