...
export default function Dashboard() {
    ...
    const [inputId, setInputId] = useState(""); // สำหรับกรอกเลขบัตรประชาชน
    const [adminId, setAdminId] = useState(null); // สำหรับเก็บข้อมูลเลขบัตรประชาชนที่ถูกตรวจสอบ

    // ดึงข้อมูล admin จาก API
    const fetchAdminData = async () => {
        try {
            const response = await fetch(`http://localhost:5000/admin/get/${adminId}`, {
                method: "GET",
                headers: {
    ...
    const handleUpdate = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/admin/update/${adminId}`, {
                method: "PUT",
                headers: {
    ...
    const handleVerifyId = () => {
        ...
        setAdminId(inputId); // บันทึกหมายเลขบัตรประชาชนที่กรอก
        fetchAdminData(); // ดึงข้อมูลจาก API
    };
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col items-center p-6">
            ...
            {!adminId ? (
                <div className="w-full max-w-md bg-white shadow-md p-6 rounded-lg">
                    <h2 className="text-xl font-semibold text-gray-600 mb-4">Enter National ID to Edit Information</h2>
