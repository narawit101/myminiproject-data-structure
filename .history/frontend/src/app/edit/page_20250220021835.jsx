...
export default function CitizensPage() {
  ...
  // ✅ โหลดข้อมูลจาก API พร้อมกรองข้อมูลซ้ำ
  const fetchCitizens = async () => {
    try {
      ...
      // ✅ กรองข้อมูลที่ไม่ซ้ำตาม citizen_id
      const uniqueCitizens = data.filter(
        (citizen, index, self) =>
          index === self.findIndex((c) => c.citizen_id === citizen.citizen_id)
      );
  ...
  useEffect(() => {
    ...
    fetchCitizens();
  }, []); // ✅ ใช้ [] เพื่อให้ fetch แค่ครั้งเดียว
            ...
            {citizens.map((citizen) => (
              <tr key={citizen.citizen_id} className="border-b hover:bg-gray-50">
                ...
                <td className="border p-3">
                  {Math.floor(citizen.income).toLocaleString()} บาท
                </td>
                <td className="border p-3">
                  {citizen.target_group || "ไม่มีข้อมูล"}
                </td>
                <td className="border p-3">
                  {citizen.distribution_date
                    ? new Date(citizen.distribution_date).toLocaleDateString(
                        "th-TH"
                      )
                    : "ไม่มีข้อมูล"}
                </td>
