...
export default function CitizensPage() {
  ...
  const fetchCitizens = async () => {
    try {
      const res = await fetch("http://localhost:5000/check-target");  // ใช้ endpoint ใหม่
      if (!res.ok) throw new Error("Failed to fetch");
      const data = await res.json();
      setCitizens(data);  // เก็บข้อมูลประชาชนทั้งหมด
    } catch (error) {
      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-200">
            ...
            <th className="border p-2">วันที่แจกจ่ายเงิน</th>
        ...
        <tbody>
          {citizens.map((citizen) => (
            <tr key={citizen.citizen_id} className="border">
              ...
              <td className="border p-2">{citizen.target_group}</td>
              <td className="border p-2">{citizen.distribution_date}</td>
