...
export default function TransactionManagement() {
  ...
  const [filterGroup, setFilterGroup] = useState("all"); // ฟิลเตอร์กลุ่มเป้าหมาย
  ...
  const filteredTransactions = transactions
    .filter((item) => (filterGroup === "all" ? true : item.group_name === filterGroup))
    .map((item, index) => ({ ...item, citizen_id: index + 1 })); // รีเซ็ต citizen_id ใหม่
    ...
    <ProtectedRoute>
      ...
      <div className="p-6 bg-gray-100 min-h-screen">
        ...
        <div className="mb-4 flex gap-2">
          <label className="mr-2">กรองกลุ่มเป้าหมาย:</label>
          <select
          ...
          >
            ...
            <option value="ผู้รายได้น้อย">ผู้รายได้น้อย</option>
            ...
            <tr className="bg-gray-200">
              <th className="border p-2 text-center">
                <input
                  ...
                  onChange={(e) =>
                    setSelectedTransactions(
                      e.target.checked ? filteredTransactions.map((item) => item.transaction_id) : []
                    )
                  }
                  checked={
