...
export default function TransactionManagement() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/transaction/get")
      ...
      .then((data) => {
        ...
        // กำหนดเลขลำดับใหม่ตาม index
        const transactionsWithId = data.map((item, index) => ({
          ...item,
          indexId: index + 1, // เริ่มที่ 1
        }));
  ...
  const handleStatusChange = async (
    ...
    newStatus,
    targetGroupId,
    budgetId // เพิ่ม parameter budgetId
  ) => {
    console.log("📤 กำลังส่งข้อมูล:", {
      ...
      newStatus,
      targetGroupId,
      budgetId, // เพิ่ม budgetId
    });
  
    if (!transactionId || !citizenId || !newStatus || !targetGroupId || !budgetId) { // ตรวจสอบว่า budgetId มีหรือไม่
    ...
    try {
      const response = await fetch("http://localhost:5000/transaction/update", {
        ...
        body: JSON.stringify({
          ...
          budget_id: budgetId, // ส่ง budget_id ไปด้วย
        }),
      });
  ...
  const filteredTransactions = transactions
    ...
    .sort((a, b) => a.indexId - b.indexId); // ✅ เรียงลำดับ ID ใหม่
          ...
          <table className="w-full border-collapse border border-gray-300 bg-white shadow-lg rounded-lg">
            <thead>
              <tr className="bg-blue-500 text-white text-left">
                <th className="border p-4">ลำดับ</th>
            ...
            <tbody>
              {filteredTransactions.length > 0 ? (
                filteredTransactions.map((item) => (
                  ...
                  >
                    <td className="p-4">{item.indexId}</td>
  ...
  onClick={() =>
    handleStatusChange(
      ...
      item.budget_id // ส่ง budget_id ไปด้วย
    )
  }
>
  เปลี่ยนสถานะ
