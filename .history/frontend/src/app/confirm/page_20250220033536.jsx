...
export default function TransactionManagement() {
  ...
  const [selectedTransactions, setSelectedTransactions] = useState([]); // ✅ เก็บธุรกรรมที่ถูกเลือก
    ...
    try {
      const response = await fetch("http://localhost:5000/transaction/update", {
        ...
        body: JSON.stringify({
          ...
          status: newStatus,
          target_group_id: transactions[0]?.target_group_id, // ใช้ค่าแรกเพราะทุกอันเหมือนกัน
        }),
      });
      ...
      if (response.ok) {
        ...
        setSelectedTransactions([]); // เคลียร์ตัวเลือก
      } else {
        ...
        <table className="w-full border-collapse border border-gray-300 bg-white shadow-md">
          <thead>
            <tr className="bg-gray-200">
              <th className="border p-2 text-center">เลือก</th>
