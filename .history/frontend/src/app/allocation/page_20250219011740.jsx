...
export default function BudgetAllocation() {
  ...
  const handleSave = (budgetId, index) => {
    ...
    const updatedData = {
      budget_id: budgetId,
      target_group_id: targetGroupId, // ✅ ดึงจาก allocations โดยตรง
      allocation_percentage: Number(editedAllocations[`${budgetId}-${index}`]?.allocation_percentage) || allocations[budgetId].allocations[index].allocation_percentage,
      max_recipients: Number(editedAllocations[`${budgetId}-${index}`]?.max_recipients) || allocations[budgetId].allocations[index].max_recipients,
    };
