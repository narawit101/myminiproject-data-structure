...
export default function BudgetAllocation() {
  ...
  <tr className="bg-blue-500 text-white">
    ...
    <th className="py-2 px-4 text-left">จำกัดผู้รับสิทธิ์</th>
    <th className="py-2 px-4 text-left">จำนวนผู้รับสิทธิ์</th>
  ...
  {allocations[budgetId].allocations.map((allocation, index) => (
    <tr key={index} className="border-t hover:bg-gray-50">
      ...
      <td className="py-2 px-4 text-right">
        <input
          type="number"
          className="w-[80px] p-2 border rounded-md text-right focus:outline-none focus:ring-2 focus:ring-blue-400"
          value={
            editedAllocations[`${budgetId}-${index}`]?.allocation_percentage ??
      ...
      <td className="py-2 px-4 text-right">
        {Number(allocation.allocated_amount).toLocaleString()} บาท
      </td>
      <td className="py-2 px-4 text-right">
        <input
          type="number"
          className="w-[100px] p-2 border rounded-md text-right focus:outline-none focus:ring-2 focus:ring-blue-400"
          value={
      ...
      <td className="py-2 px-4 text-right">
        {Number(allocation.max_recipients).toLocaleString()} คน
      </td>
      <td className="py-2 px-4 text-right">
        {Number(allocation.amount_per_person).toLocaleString()} บาท
      </td>
