...
export default function BudgetAllocation() {
  ...
  const handleSave = (budgetId, index) => {
    ...
    const totalPercentage = allocations[budgetId].allocations.reduce((sum, item, i) => {
        const itemPercentage = i === index ? allocationPercentage : Number(item.allocation_percentage);
        return sum + itemPercentage;
    }, 0);

    if (totalPercentage > 100) {
        console.error(`❌ การจัดสรรงบประมาณเกิน 100%! (ปัจจุบัน ${totalPercentage}%)`);
        return;
    }
    ...
    fetch("http://localhost:5000/allocation/update", {
        ...
        .then((data) => {
            ...
            // ✅ อัปเดต state ให้ UI เปลี่ยนโดยไม่ต้องรีเฟรช
            setAllocations((prev) => {
            ...
            // ✅ เคลียร์ค่า editedAllocations เพื่อให้ค่าที่แก้ไขกลับเป็นปกติ
            setEditedAllocations((prev) => {
            ...
            // ✅ Force re-render เพื่อให้ UI อัปเดต
