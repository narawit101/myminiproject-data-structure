    ...
    fetch("http://localhost:5000/allocation/get")
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          // ✅ ถ้าเป็นอาร์เรย์ จะทำการจัดกลุ่มข้อมูลตาม budget_id
          ...
          // ✅ ถ้า data เป็นอ็อบเจ็กต์ที่มีคีย์ allocations เป็นอาร์เรย์
          const groupedData = data.allocations.reduce((acc, item) => {
            if (!acc[item.budget_id]) {
              acc[item.budget_id] = {
                project_name: item.project_name,
                allocations: [],
          ...
          // ❌ ถ้าไม่ใช่ทั้งอาร์เรย์หรืออ็อบเจ็กต์ที่มีคีย์ allocations
