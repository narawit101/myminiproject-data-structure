...
async function generateFakeCitizens(count = 10) {
    try {
        for (let i = 0; i < count; i++) {
            ...
            const national_id = faker.number.int({ min: 5000000000000, max: 5999999999999 }).toString();
            const birth_date = faker.date.between({ from: "1960-01-01", to: "2005-12-31" });
            const age = new Date().getFullYear() - birth_date.getFullYear();
            const income = faker.number.int({ min: 3000, max: 50000 }).toFixed(2);
            const occupation = faker.helpers.arrayElement(["เกษตรกร", "พนักงานบริษัท", "ข้าราชการ", "อาชีพอิสระ"]);
