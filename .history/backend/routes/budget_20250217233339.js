...
router.put("/update/:id", async (req, res) => {
    const { id } = req.params;
    const { project_name, total_budget } = req.body; // ✅ รับค่ามาทั้งคู่

    try {
        console.log("Update Request:", { id, project_name, total_budget }); // ✅ Debug

        if (!id || !project_name || total_budget === undefined) {
            return res.status(400).json({ message: "Missing required fields" });
        }

        // ✅ อัปเดต project_name และ total_budget
        const result = await pool.query(
            "UPDATE budgetconfig SET project_name = $1, total_budget = $2, updated_at = NOW() WHERE id = $3 RETURNING *",
            [project_name, total_budget, id]
        );

        console.log("SQL Result:", result.rows); // ✅ Debug ดูค่าที่อัปเดต
