...
router.post("/add", async (req, res) => {
    try {
        const { budget_id, target_group_id, allocation_percentage, max_recipients } = req.body;

        if (!budget_id || !target_group_id || allocation_percentage == null || max_recipients == null) {
            return res.status(400).json({ error: "กรุณาระบุ budget_id, target_group_id, allocation_percentage และ max_recipients" });
        }
        ...
        if (newTotal > 100) {
            return res.status(400).json({ error: `รวมเปอร์เซ็นต์เกิน 100% (${newTotal}%)` });
        }
        ...
        await pool.query(
            `INSERT INTO AllocationRatio (budget_id, target_group_id, allocation_percentage, max_recipients, allocated_amount)
             VALUES ($1, $2, $3, $4, $5)`,
            [budget_id, target_group_id, allocation_percentage, max_recipients, allocated_amount]
        );
        ...
        const totalAllocatedAmount = parseFloat(totalAllocatedResult.rows[0].total_allocated);
        ...
        res.json({
            ...
            remaining_budget: parseFloat(updateBudget.rows[0].remaining_budget).toFixed(2)
        });
...
router.put("/update", async (req, res) => {
    try {
        ...
        await pool.query(
            ...
            [allocation_percentage, max_recipients, new_allocated_amount, budget_id, target_group_id]
        );
...
router.get("/get/:budget_id", async (req, res) => {
    try {
        ...
        if (allocationResult.rowCount === 0) {
            return res.status(404).json({ error: "ไม่พบข้อมูลการจัดสรรสำหรับ budget_id นี้" });
        }
