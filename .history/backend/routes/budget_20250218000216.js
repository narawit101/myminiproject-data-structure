...
router.put("/update/:id",authenticate, async (req, res) => {
    try {
        const { budget_id, total_budget } = req.body;

        // ✅ ตรวจสอบว่ามีข้อมูลครบหรือไม่
        if (!budget_id || !total_budget) {
            return res.status(400).json({ error: "กรุณาระบุ budget_id และ total_budget" });
        }

        // ✅ อัปเดตงบประมาณ
        const result = await pool.query(
            `UPDATE BudgetConfig SET total_budget = $1, remaining_budget = $1, updated_at = NOW() 
             WHERE id = $2 RETURNING *`,
            [total_budget, budget_id]
        );
