    ...
    try {
        ...
        const allocationResult = await pool.query(`
            SELECT 
                ...
                ar.allocated_amount,
                COALESCE(SUM(CASE WHEN t.status = 'completed' THEN t.amount ELSE 0 END), 0) AS total_paid, -- ✅ รวมเงินที่จ่ายไปแล้ว
                (ar.allocated_amount - COALESCE(SUM(CASE WHEN t.status = 'completed' THEN t.amount ELSE 0 END), 0)) AS remaining_budget, -- ✅ เงินที่เหลือจริง
