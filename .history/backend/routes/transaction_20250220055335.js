...
router.put("/update", async (req, res) => {
  try {
      const { transaction_id, citizen_id, status, target_group_id, budget_id } = req.body;

      if (!transaction_id || !citizen_id || !status || !target_group_id || !budget_id) {
          return res.status(400).json({ error: "กรุณาระบุ transaction_id, citizen_id, status, target_group_id และ budget_id" });
      }
      ...
      if (status === "completed") {
          ...
          if (allocationResult.rowCount === 0) {
              return res.status(400).json({ error: "ไม่พบข้อมูลการจัดสรรสำหรับกลุ่มเป้าหมายนี้" });
          }

          const { allocated_amount, max_recipients } = allocationResult.rows[0];
          const amount_per_person = max_recipients > 0 ? allocated_amount / max_recipients : 0;
          ...
          if (allocated_amount >= amount_per_person && amount_per_person > 0) {
              ...
              return res.status(400).json({ error: "งบประมาณไม่เพียงพอที่จะจ่ายให้กับกลุ่มเป้าหมายนี้" });
          }
      }
