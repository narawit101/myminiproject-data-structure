...
router.put("/update/:id", async (req, res) => {
    const { id } = req.params;
    const { project_name, total_budget } = req.body; // ✅ รับค่าจาก Frontend

    try {
        console.log("Update Request:", { id, project_name, total_budget }); // ✅ Debug ดูค่าที่ได้รับ

        if (!id || !project_name || total_budget === undefined) {
            return res.status(400).json({ message: "Missing required fields" });
        }

        // ✅ อัปเดตทั้ง project_name และ total_budget
