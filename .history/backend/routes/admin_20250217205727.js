...
router.get("/:national_id", authenticate, async (req, res) => {
    const { national_id } = req.params;
    console.log("รับค่าบัตรประชาชน:", national_id); // ✅ DEBUG ตรงนี้
    ...
    try {
      ...
      if (result.rowCount === 0) {
        console.log("ไม่พบข้อมูลใน DB"); // ✅ DEBUG
        return res.status(404).json({ message: "ไม่พบข้อมูลประชาชน" });
      }
  
      res.json(result.rows[0]); // ✅ ส่งข้อมูลกลับ
    } catch (error) {
