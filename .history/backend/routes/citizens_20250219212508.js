...
router.post("/post", async (req, res) => {
  try {
    ...
    // ✅ ตรวจสอบว่า national_id ซ้ำหรือไม่
    ...
    if (target_group_id) {
      // ✅ ดึงงบประมาณที่สัมพันธ์กับ target_group_id
      const budgetResult = await pool.query(
        `SELECT b.id, a.allocation_percentage, a.max_recipients, b.total_budget 
         FROM BudgetConfig b
         JOIN AllocationRatio a ON b.id = a.budget_id
         WHERE b.remaining_budget > 0 
         AND a.target_group_id = $1
      ...
      if (budgetResult.rowCount > 0) {
        ...
        const transactionResult = await pool.query(
          `INSERT INTO Transaction (citizen_id, amount, transaction_date, status, queue_order, allocation_id)
           ...
           RETURNING *`,
          [citizen.citizen_id, amount_per_person, budget_id]
        );
