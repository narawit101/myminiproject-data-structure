...
router.post("/add", async (req, res) => {
    try {
        ...
        // ✅ ตรวจสอบว่าเปอร์เซ็นต์ต้องไม่ติดลบ
        if (percentage < 0) {
            return res.status(400).json({ error: "ค่า allocation_percentage ต้องไม่น้อยกว่า 0" });
        }

        // ✅ ดึงข้อมูลงบประมาณเพื่อตรวจสอบ max_allocations
        const budgetResult = await pool.query(
            "SELECT project_name, total_budget, max_allocations FROM BudgetConfig WHERE id = $1",
            [budget_id]
        );

        // ✅ ถ้าไม่พบ budget_id ให้แจ้งว่าไม่พบข้อมูล
        if (budgetResult.rowCount === 0) {
            return res.status(404).json({ error: "ไม่พบบัญชีงบประมาณที่ระบุ" });
        }

        const { project_name, total_budget, max_allocations } = budgetResult.rows[0];

        // ✅ ตรวจสอบจำนวนปัจจุบันของ AllocationRatio
        const allocationCountResult = await pool.query(
            "SELECT COUNT(*) AS count FROM AllocationRatio WHERE budget_id = $1",
            [budget_id]
        );

        const currentAllocations = parseInt(allocationCountResult.rows[0].count, 10);

        // ✅ ถ้าถึงจำนวนสูงสุดแล้ว ห้ามเพิ่มข้อมูลใหม่
        if (currentAllocations >= max_allocations) {
            return res.status(400).json({ error: `ไม่สามารถเพิ่มข้อมูลได้เกิน ${max_allocations} กลุ่ม` });
        }
        ...
        await pool.query(
            "INSERT INTO AllocationRatio (budget_id, target_group_id, allocation_percentage) VALUES ($1, $2, $3)",
            [budget_id, target_group_id, percentage]
        );
