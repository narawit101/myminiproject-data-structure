...
router.get("/check-target", async (req, res) => {
  try {
    const citizenQuery = `
      SELECT c.citizen_id, c.fname, c.lname, c.birth_date, c.age, c.income, c.occupation,
    ...
    res.status(500).json({ message: "เกิดข้อผิดพลาดในการตรวจสอบ", error: error.message });  // ส่งข้อความผิดพลาดกลับ
  }
});
...
router.get("/check-target", async (req, res) => {
  try {
    // ดึงข้อมูลของประชาชนทั้งหมดพร้อมกลุ่มเป้าหมายและการแจกจ่ายเงิน
    const citizenQuery = `
      SELECT 
        c.national_id,
        c.fname, 
        ...
        c.income, 
        c.occupation, 
        tg.name AS target_group,   // ชื่อกลุ่มเป้าหมาย
        fds.distribution_date     // วันที่แจกจ่ายเงิน
      FROM Citizens c
      LEFT JOIN TargetGroup tg ON c.target_group_id = tg.target_group_id
      LEFT JOIN FundDistributionSchedule fds ON tg.target_group_id = fds.target_group_id;
    `;

    // รันคำสั่ง SQL เพื่อตรวจสอบข้อมูล
    const { rows } = await pool.query(citizenQuery);

    // หากไม่พบข้อมูลประชาชนให้แสดงข้อผิดพลาด 404
    if (rows.length === 0) {
      return res.status(404).json({ message: "ไม่พบข้อมูลประชาชน" });
    }

    // ส่งข้อมูลทั้งหมดกลับในรูปแบบ JSON
    res.json(rows);  // ส่งข้อมูลประชาชนทั้งหมดพร้อมกลุ่มเป้าหมาย
  } catch (error) {
...
router.get("/check-target/:national_id", async (req, res) => {
  ...
  try {
    ...
    if (citizenData.birth_date) {
      citizenData.birth_date = new Date(citizenData.birth_date).toISOString().split("T")[0]; // YYYY-MM-DD
    }

    if (citizenData.distribution_date) {
      citizenData.distribution_date = new Date(citizenData.distribution_date).toLocaleDateString("th-TH", {
        year: "numeric",
        month: "long",
        day: "numeric",
