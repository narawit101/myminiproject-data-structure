...
router.put("/update", async (req, res) => {
  try {
    ...
    // ✅ คำนวณจำนวนเงินต่อคนใหม่
const amount_per_person = new_allocated_amount / max_recipients;

// ✅ อัปเดต Transaction
// ✅ อัปเดต allocation_id ใน Transaction (ถ้ามี)
await pool.query(
  `UPDATE Transaction 
   SET allocation_id = (SELECT id FROM AllocationRatio WHERE budget_id = $1 AND target_group_id = $2)
   WHERE citizen_id IN 
       (SELECT citizen_id FROM EligibleRecipients WHERE target_group_id = $2) 
     AND status = 'pending'`,
  [budget_id, target_group_id]
);
