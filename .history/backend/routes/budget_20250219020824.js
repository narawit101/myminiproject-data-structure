...
router.get('/get', authenticate, async (req, res) => {
    try {
      ...
      const budgets = result.rows.map(budget => {
        ...
        const remainingBudget = budget.total_budget - (budget.spent || 0); // หักค่าใช้จ่ายออกจากงบประมาณทั้งหมด
        return {
          ...budget,
          remaining_budget: remainingBudget // เพิ่ม remaining_budget ในผลลัพธ์
        };
      });
  
      res.json(budgets); // ส่งข้อมูลที่คำนวณเสร็จแล้ว
    } catch (err) {
...
router.post("/add", authenticate, async (req, res) => {
  try {
    ...
    if (!year || !project_name || !total_budget) {
      return res
        .status(400)
        .json({ error: "กรุณาระบุปี, ชื่อโครงการ และงบประมาณ" });
    }
...
router.put("/update/:budget_id", async (req, res) => {
  try {
    ...
    if (!budget_id || !project_name || !total_budget) {
      return res
        .status(400)
        .json({ error: "กรุณาระบุ budget_id, project_name และ total_budget" });
    }
