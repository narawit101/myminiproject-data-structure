...
router.post("/post", async (req, res) => {
  try {
    ...
    // ✅ ค้นหากลุ่มเป้าหมายที่มีสิทธิ์รับเงิน
    ...
    let target_group_id = groupQuery.rowCount > 0 ? groupQuery.rows[0].target_group_id : null;
    ...
    if (target_group_id) {
      // ✅ ค้นหางบประมาณล่าสุดที่ยังมีงบเหลือ
      const budgetResult = await pool.query(
        `SELECT id, total_budget FROM BudgetConfig WHERE remaining_budget > 0 ORDER BY year DESC LIMIT 1`
      );

      if (budgetResult.rowCount > 0) {
        const { id: budget_id, total_budget } = budgetResult.rows[0];

        // ✅ ค้นหา allocation_id ที่สัมพันธ์กับ target_group_id
        const allocationResult = await pool.query(
          `SELECT id, allocation_percentage, max_recipients FROM AllocationRatio 
           WHERE budget_id = $1 AND target_group_id = $2 LIMIT 1`,
          [budget_id, target_group_id]
        );

        if (allocationResult.rowCount > 0) {
          const { id: allocation_id, allocation_percentage, max_recipients } = allocationResult.rows[0];
          ...
          return res.status(400).json({ message: "ไม่พบ AllocationRatio ที่เกี่ยวข้อง" });
        }
      }
    }
