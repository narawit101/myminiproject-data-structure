...
router.post('/post', async (req, res) => {
    try {
        ...
        // ✅ ถ้าอาชีพเป็น "เกษตรกร" ให้กำหนด target_group_id เป็นกลุ่มที่เหมาะสม
        if (occupation.includes("เกษตรกร")) {
            const farmerQuery = await pool.query(
                SELECT target_group_id FROM TargetGroup WHERE name ILIKE '%เกษตรกร%' LIMIT 1
            );
            target_group_id = farmerQuery.rowCount > 0 ? farmerQuery.rows[0].target_group_id : null;
        }
        
        // ✅ ถ้าไม่ใช่ "เกษตรกร" คำนวณ target_group_id ตามอายุและรายได้
        if (!target_group_id) {
